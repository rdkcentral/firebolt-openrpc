...
import fs from 'fs'
import { getProvidedCapabilities, isRPCOnlyMethod, isTemporalSetMethod, isProviderInterfaceMethod, getSemanticVersion } from '../../shared/modules.mjs'
...
const insertAggregateMacros = content => content
...
function generateMacros(module, templates) {
    return {
        module,
        templates
    }
...
function insertMacros(data = '', macros = {}, options = {}) {
    ...
    const version = getSemanticVersion(module)
    const methods = module.methods && module.methods.filter( method => !isEvent(method) && !isRPCOnlyMethod(method) )
    const events = module.methods && module.methods.filter( method => isEvent(method) && isFullyDocumentedEvent(method))
    const additionalEvents = module.methods && module.methods.filter( method => isEvent(method) && !isFullyDocumentedEvent(method))
    const additionalMethods = module.methods && module.methods.filter( method => !isEvent(method) && isRPCOnlyMethod(method) )
    ...
    const providerMethods = module.methods && module.methods.filter( method => isProviderInterfaceMethod(method))
    const hasProviderMethods = (providerMethods && providerMethods.length > 0)

    const capabilities = module.methods && getProvidedCapabilities(module)
    ...
    if (capabilities) {
        ...
        if (match = data.match(regex)) {
            ...
            capabilities.forEach(capability => {
                providers += insertProviderInterfaceMacros(match[0], capability, module, templates, options)
            })
    ...
    if (events && events.length > 0) {
        ...
        if (match = data.match(regex)) {
            ...
            events.forEach(method => {
                eventsBlock += insertMethodMacros(match[0], method, module, templates, options)
            })
    ...
    if (methods && methods.length > 0) {
        ...
        while (match = data.match(regex)) {
            ...
            methods.forEach(method => {
                if (isTocMethod(method)) {
                    methodsStr += insertMethodMacros(match[0], method, module, templates, options)
                }
            })
        ...
        while (match = data.match(regex)) {
            data = data.replace(regex, insertEventMacros(match[0], methods, module, null, templates, options))
        }
    ...
    if (additionalEvents && additionalEvents.length > 0) {
        ...
        while (match = data.match(regex)) {
            ...
            additionalEvents.forEach(event => {
                ...
                additionalStr += insertMethodMacros(match[0], event, module, templates, options)
            })
    ...
    if (additionalMethods && additionalMethods.length > 0) {
        ...
        while (match = data.match(regex)) {
            ...
            additionalMethods.forEach(method => {
                ...
                additionalStr += insertMethodMacros(match[0], method, module, templates, options)
            })
            data = data.replace(regex, additionalStr)
        }
    }
    ...
    if (module.components && module.components.schemas && Object.values(module.components.schemas).length) {
        local_schemas = module.components.schemas
        prefix = '#/components/schemas/'
    }
    else if (module.definitions && Object.values(module.definitions).length) {
        local_schemas = module.definitions
        prefix = '#/definitions/'
    }
    ...
    if (local_schemas) {
        ...
        while (match = data.match(regex)) {
            data = data.replace(regex, insertSchemaMacros(match[0], local_schemas, module, null, options))
        }
        let schemas_toc = []
        Object.entries(local_schemas).forEach(([n, v]) => {
            if (true || isDefinitionReferencedBySchema(prefix + n, module)) {
    ...
    if (methods) {
        data = data
            ...
            .replace(/\$\{toc.providers\}/g, capabilities.map(c => `    - [${getProviderName(c, module, null)}](#${getProviderName(c, module, null).toLowerCase()})`).join('\n'))
    }

    const providerSessionInterface = getProviderSessionInterface(module)

    data = data
        .replace(/\$\{module}/g, getTitle(module).toLowerCase() + '.json')
        .replace(/\$\{info.title}/g, getTitle(module))
        .replace(/\$\{jsonrpc.module}/g, getTitle(module).toLowerCase())
        ...
        .replace(/\$\{info.description}/g, module.info && module.info.description || '')
...
function insertProviderInterfaceMacros(data, capability, moduleJson = {}, templates = {}, options = {}) {
    let result = ''
    ...
    const iface = getProviderInterface(moduleJson, capability, null)//.map(method => { method.name = method.name.charAt(9).toLowerCase() + method.name.substr(10); return method } )
    ...
    interfaceShape += iface.map(method => `\t${getMethodSignature(moduleJson, method, null, { isInterface: true })}`).join('\n')
    interfaceShape += '\n}\n'

    let propertiesShape = ''
    propertiesShape += `{\n`
    propertiesShape += iface.map(method => `\t${getMethodSignature(moduleJson, method, null, { isInterface: false })}`).map(str => str.replace(/function (.*?)\(/, '$1: function(')).join('\n')
    propertiesShape += '\n}\n'

    // todo generate example vanilla JS class that returns example result wrapped in promise
    let exampleClass = ''
    exampleClass += `interface ${name} {\n`
    exampleClass += iface.map(method => `\t${getMethodSignature(moduleJson, method, null, { isInterface: true })}`).join('\n')
    exampleClass += '\n}\n'
    ...
    if (iface.length === 0) {
        ...
        if (match) {
            iface.forEach(method => {
                ...
                const parametersShape = getSchemaShape(moduleJson, parametersSchema, null)
                let methodBlock = insertMethodMacros(match[0], method, moduleJson, templates, options)
                ...
                if (hasProviderParameters) {
                    ...
                    for (let i = lines.length - 1; i >= 0; i--) {
                        if (lines[i].match(/\$\{provider\.param\.[a-zA-Z]+\}/)) {
                            let line = lines[i]
                            lines.splice(i, 1)
                            line = insertProviderParameterMacros(line, method.params[0].schema, moduleJson, null, options)
...
function getSeeAlsoLink(method, moduleJson) {
    ...
    const provides = respondsTo && getProviderName(getExtensionValue(respondsTo, 'x-provides'), moduleJson)
...
function insertMethodMacros(data, method, moduleJson = {}, templates = {}, options = {}) {
    let result = ''
    ...
    data = iterateSignatures(data, method, moduleJson, templates, options)
    ...
    const seeAlsoLink = getSeeAlsoLink(method, moduleJson)
...
function iterateSignatures(data, method, moduleJson = {}, templates = {}, options = {}) {
    ...
    if (method.tags && method.tags.find(t => t.name === 'polymorphic-pull')) {
        ...
        const pullParameters = localizeDependencies(
          ...
          moduleJson
        ) || getExternalPath('#/definitions/' + method.name[0].toUpperCase() + method.name.substr(1), null, true)
    ...
    signatures.forEach(sig => {
        ...
        let block = sig == null ? '' : insertSignatureMacros(match[1], sig, moduleJson, templates, options)
...
function insertSignatureMacros(block, sig, moduleJson = {}, templates = {}, options = {}) {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
        if (lines[i].match(/\$\{method\.param\.[a-zA-Z]+\}/)) {
            let line = lines[i]
            lines.splice(i, 1)
            sig.params.forEach((param) => { lines.splice(i++, 0, insertParamMacros(line, param, moduleJson, options)) })
        }
    }
    ...
    block = block.replace(/\$\{method.signature\}/g, getMethodSignature(moduleJson, sig, null, { isInterface: false }))
        .replace(/\$\{method.params\}/g, getMethodSignatureParams(moduleJson, sig))
        ...
        .replace(/\$\{method.result.link\}/g, getSchemaType(moduleJson, sig.result, null, {title: true, link: true, asPath: options.asPath, baseUrl: options.baseUrl}))
        .replace(/\$\{method.result.type\}/g, getSchemaType(moduleJson, sig.result, null, {title: true, asPath: options.asPath, baseUrl: options.baseUrl}))
        .replace(/\$\{event.result.type\}/, getSchemaType(moduleJson, (sig.result.schema.anyOf || [sig.result.schema]).pop(), null, { event: true, description: sig.result.summary, asPath: options.asPath, baseUrl: options.baseUrl })) //getType(method.result, true))
        .replace(/\$\{method.result\}/g, getSchemaTypeTable(moduleJson, sig.result, { description: sig.result.summary, title: true, asPath: options.asPath, baseUrl: options.baseUrl}))

    return block
}

function insertSchemaMacros(data, local_schemas = {}, moduleJson = {}, options = {}) {
    let result = ''
    ...
    Object.entries(local_schemas).forEach(([name, schema]) => {
        if (true || isDefinitionReferencedBySchema(prefix + name, (moduleJson.methods ? moduleJson.methods : moduleJson)) || schema.title) {
            ...
            lines = lines.split('\n')
            const schemaShape = getSchemaShape(moduleJson, schema, null, name)
...
function insertEventMacros(data = '', methods = [], moduleJson = {}, templates = {}, options = {}) {
    let result = ''
    ...
    methods.forEach(method => {
        ...
        let method_data = lines.join('\n')
            ...
            .replace(/\$\{event.result.type\}/, getSchemaTypeTable(moduleJson, method.result.schema.anyOf.pop(), { event: true, description: method.result.summary, asPath: options.asPath, baseUrl: options.baseUrl })) //getType(method.result, true))
...
function insertParamMacros(data = '', param, module = {}, options = {}) {
    let constraints = getSchemaConstraints(param, module, null)
    let type = getSchemaType(module, param, null, { code: true, link: true, title: true, asPath: options.asPath, baseUrl: options.baseUrl })
...
function getSchemaTypeTable(module, moduleJson = {}, options = {}) {
    let type = getSchemaType(module, moduleJson, null, options)
    ...
    if (type === 'object' && moduleJson.properties) {
        ...
        Object.entries(moduleJson.properties).forEach(([name, prop]) => {
            type += `| \`${name}\` | ${getSchemaType(module, prop, null, { link: true, code: true, event: options.event, asPath: options.asPath, baseUrl: options.baseUrl }).replace('|', '\\|')} | ${prop.description || ''} |\n`
        })
        ...
        const path = obj['$ref']
        const ref = path ? getPath(path, module,) : moduleJson
        const schemaType = getSchemaType(module, moduleJson, null, { code: true, link: true, event: options.event, title: true, asPath: options.asPath, baseUrl: options.baseUrl })
...
function getExternalSchemaLinks(json = {}, options = {}) {
    ...
    let links = getExternalSchemaPaths(json)
        ...
        .map(path => options.baseUrl + getLinkFromRef(path, null, options.asPath))
