...
import { isExcludedMethod, isRPCOnlyMethod, isProviderInterfaceMethod, getPayloadFromEvent, providerHasNoParameters, isTemporalSetMethod, hasMethodAttributes, getMethodAttributes, isEventMethodWithContext, getSemanticVersion, getSetterFor } from '../../shared/modules.mjs'
...
const generateMacros = (obj, templates) => {
  ...
  const examples = generateExamples(obj,templates)
...
const insertMacros = (fContents = '', macros = {}) => {
  ...
  examples.forEach((match) => {
    fContents = fContents.replace(match[0], JSON.stringify(macros.examples[match[1]].result.value))
  })
...
const generateImports = json => {
  ...
  if (providersOrEmptyArray(json).length) {
    imports += `import ProvideManager from '../ProvideManager/index.mjs'\n`
    imports += `import { registerProviderInterface } from \'../ProvideManager/index.mjs\'\n`
  }
...
function generateExamples(json = {}, templates = {}) {
  const examples = {}
  json && json.methods && json.methods.forEach(method => {
    examples[method.name] = JSON.parse(JSON.stringify(method.examples[0]))
  })
...
function generateMethods(json = {}, templates = {}, onlyEvents = false) {
  ...
  const methodReduction = reduce((acc, methodObj, i, arr) => {
      ...
      if (isPropertyMethod(methodObj)) {
        template = getTemplate('methods', 'polymorphic-property.js', templates)
      }
      else if (isEventMethod(methodObj) && methodObj.params.length > 1) {
        template = getTemplate('methods', 'event-with-context.js', templates)
