...
const run = async ({
    ...
    if (input) {
        ...
        logHeader(`Validating ${path.relative('.', platformApi)} with${transformations ? '' : 'out'} Firebolt transformations.`)
    }
    ...
    const moduleList = input ? await readDir(path.join(input), { recursive: true }) : [platformApi]
    ...
    Object.keys(modules).forEach(key => {
        ...
        try {
            ...
            if (false && passThroughs) {
                console.log("Pass-through disabled in major/rpc-deprecated");
