...
function insertMacros(data = '', moduleJson = {}, templates = {}, options = {}) {
    let match, regex
    const methods = compose(
      option([]),
      map(filter(method => !isEvent(method) && !isSetter(method) || isEvent(method) && isFullyDocumentedEvent(method))),
      safe(isArray)
    )(moduleJson.methods)

    const additionalEvents = compose(
      option([]),
      map(filter(method => isEvent(method) && !isFullyDocumentedEvent(method))),
      safe(isArray)
    )(moduleJson.methods)
    ...
    if (methods) {
        ...
        if (match = data.match(/\$\{events\}/)) {
            let events = ''
            methods.forEach(method => {
                if (method.tags && method.tags.find(t => t.name === 'event')) {
                    events += insertMethodMacros(match[0], method, moduleJson, templates, options)
                }
            })
        ...
        while (match = data.match(regex)) {
            ...
            methods.forEach(method => {
                if (!method.tags || (!method.tags.find(t => t.name === 'event'))) {
                    methodsStr += insertMethodMacros(match[0], method, moduleJson, templates, options)
                }
            })
        ...
        while (match = data.match(regex)) {
            data = data.replace(regex, insertEventMacros(match[0], methods, moduleJson, templates, options))
        }
    ...
    if (additionalEvents && additionalEvents.length > 0) {
        ...
        while (match = data.match(regex)) {
            ...
            additionalEvents.forEach(event => {
                ...
                additionalStr += insertMethodMacros(match[0], event, moduleJson, templates, options)
            })
    ...
    if (schemas) {
        ...
        while (match = data.match(regex)) {
            data = data.replace(regex, insertSchemaMacros(match[0], schemas, moduleJson, options))
        }
...
function iterateSignatures(data, method, moduleJson = {}, templates = {}, options = {}) {
    ...
    if (method.tags && method.tags.find(t => t.name === 'polymorphic-pull')) {
        ...
        const pullParameters = localizeDependencies(getPath('#/components/schemas/' + method.name[0].toUpperCase() + method.name.substr(1) + 'Parameters', moduleJson), moduleJson) || getExternalPath('#/definitions/' + method.name[0].toUpperCase() + method.name.substr(1), true)
...
function insertSignatureMacros(block, sig, moduleJson = {}, templates = {}, options = {}) {
    ...
    block = block.replace(/\$\{method.signature\}/g, getMethodSignature(moduleJson, sig, { isInterface: false }))
        ...
        .replace(/\$\{method.result.link\}/g, getSchemaType(moduleJson, sig.result, {title: true, link: true, asPath: options.asPath, baseUrl: options.baseUrl}))
        .replace(/\$\{method.result.type\}/g, getSchemaType(moduleJson, sig.result, {title: true, asPath: options.asPath, baseUrl: options.baseUrl}))
...
function insertSchemaMacros(data, schemas, moduleJson = {}, options = {}) {
    let result = ''
    ...
    Object.entries(schemas).forEach(([name, schema]) => {
        if (true || isDefinitionReferencedBySchema(prefix + name, (moduleJson.methods ? moduleJson.methods : moduleJson)) || schema.title) {
            ...
            let schema_data = lines.join('\n')
                ...
                .replace(/\$\{schema.shape\}/, getSchemaShape(moduleJson, schema, name))
...
function insertEventMacros(data = '', methods = [], moduleJson = {}, templates = {}, options = {}) {
    ...
    methods.forEach(method => {
        ...
        let method_data = lines.join('\n')
            ...
            .replace(/\$\{event.result.type\}/, getSchemaTypeTable(moduleJson, method.result, { event: true, description: method.result.summary, asPath: options.asPath, baseUrl: options.baseUrl })) //getType(method.result, true))
...
function insertParamMacros(data, param, module, options = {}) {
    ...
    let type = getSchemaType(module, param, { code: true, link: true, title: true, asPath: options.asPath, baseUrl: options.baseUrl })
...
function getSchemaTypeTable(module, moduleJson = {}, options = {}) {
    let type = getSchemaType(module, moduleJson, options)
    ...
    if (type === 'object' && moduleJson.properties) {
        ...
        type += `| ${getSchemaType(module, moduleJson, { code: true, link: true, event: options.event, title: true, asPath: options.asPath, baseUrl: options.baseUrl }).replace('|', '\\|')} | ${moduleJson.description || ref.description || summary || ''} |\n`
